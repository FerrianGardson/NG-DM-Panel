...
function DMPanelNoble:OnEnable()
    ...
    if DMPanelNoble.db.profile.settings.AddNameToEmote then
        SettingsFrame.AddName:SetChecked(true)
    else
        SettingsFrame.AddName:SetChecked(false)
    end
    if DMPanelNoble.db.profile.settings.SaveFocus then
...
function DMPanelNoble:DMLevelTimer()
    if DMLevel > 0 then
        print("|cffff9716[ДМ-аддон]: Ваш текущий уровень ДМ-доступа - " .. DMLevel ..
                  "|r")
    end
    ...
    if DMLevel < 3 then
        ...
        MPA_AurasAndStatesPanel.InvisOnoff.Icon.Tooltip =
            "Только для ДМ-доступа\nтретьего уровня";
        MPA_AurasAndStatesPanel.SpeedOnOff.Icon.Tooltip =
            "Только для ДМ-доступа\nтретьего уровня";
        MPA_AurasAndStatesPanel.FlyOnOff.Icon.Tooltip =
            "Только для ДМ-доступа\nтретьего уровня";
        MPA_AurasAndStatesPanel.Phase.Icon.Tooltip =
            "Только для ДМ-доступа\nтретьего уровня";
        MPA_AurasAndStatesPanel.SetAura.Icon.Tooltip =
            "Только для ДМ-доступа\nтретьего уровня";
        MPA_AurasAndStatesPanel.ClearAura.Icon.Tooltip =
            "Только для ДМ-доступа\nтретьего уровня";
    end
end
...
MPA_MainPanel.CloseButton = CreateFrame("BUTTON", "MPA_MainPanel.CloseButton", MPA_MainPanel,
    "DMPanelNoble:CloseButtonTemplate");
...
MPA_MainPanel.RefreshButton = CreateFrame("BUTTON", "MPA_MainPanel.RefreshButton", MPA_MainPanel,
    "DMPanelNoble:RefreshButtonTemplate");
...
MPA_EditPanel.EditBox:SetScript('OnEnterPressed', function(self)
    ...
    elseif EditboxStatement == "CHATCOLORSTATE" then
        if isempty(tostring(EditboxText)) then
            ...
            if (not DMPanelNoble.db.profile.settings.ChatRadius) or DMPanelNoble.db.profile.settings.ChatRadius == nil or
                tonumber(DMPanelNoble.db.profile.settings.ChatRadius) == 0 then
                MessageStringSplitter(tostring(EditboxText), 5, 0, tonumber(Chosen_colour))
            else
                MessageStringSplitter(tostring(EditboxText), 5, tonumber(DMPanelNoble.db.profile.settings.ChatRadius),
        ...
        elseif isempty(tostring(EditboxText)) then
            ...
            if (not DMPanelNoble.db.profile.settings.ChatRadius) or DMPanelNoble.db.profile.settings.ChatRadius == nil or
                ...
                TalkingHeadRetranslator(tostring(EditboxText), GetUnitName("target"), GetUnitName("player"),
                    tonumber(DMPanelNoble.db.profile.settings.ChatRadius))
            end
        end
    ...
    elseif EditboxStatement == "SETSCALENPCSTATEMENT" then
        ...
        elseif tonumber(EditboxText) > 5 or tonumber(EditboxText) < 0.05 then
            ...
            SendChatMessage(".changescale " .. tonumber(EditboxText), "WHISPER", GetDefaultLanguage("player"),
                GetUnitName("player"));
        end
    elseif EditboxStatement == "NPCPLAYEMOTESTATEMENT" then
        ...
        elseif isempty(tostring(EditboxText)) then
            ...
            SendChatMessage(".npcplayemote " .. tostring(EditboxText), "WHISPER", GetDefaultLanguage("player"),
                GetUnitName("player"));
        end
    ...
    elseif EditboxStatement == "SUMMONSTATEMENT" then
        ...
        elseif string.lower(tostring(EditboxText)) == 1 or string.lower(tostring(EditboxText)) == "all" or
            string.lower(tostring(EditboxText)) == "raid" then
            -----
...
MPA_SelectPanel.NPCButton = CreateFrame("Button", "MPA_SelectPanel.NPCButton", MPA_SelectPanel,
    "DMPanelNoble:IconButtonBG")
MPA_SelectPanel.NPCButton:SetPoint("CENTER", -120, -18)

MPA_SelectPanel.NPCButton.Icon =
    CreateFrame("BUTTON", nil, MPA_SelectPanel.NPCButton, "DMPanelNoble:IconButtonTemplate");
...
MPA_SelectPanel.SearchAndDel.Icon = CreateFrame("BUTTON", nil, MPA_SelectPanel.SearchAndDel,
    "DMPanelNoble:IconButtonTemplate");
...
MPA_SelectPanel.Options.Icon:SetScript("OnClick", function()
    DMPanelNoble:SettingsFrameOnOff()
end)
...
MPA_NPCPanel.TalkingHead_Button = CreateFrame("BUTTON", "MPA_MainPanel.TalkingHead_Button", MPA_NPCPanel,
    "DMPanelNoble:NPCTalkingHeadButton");
...
MPA_SearchAndDelPanel.LoCrButton = CreateFrame("BUTTON", "MPA_SearchAndDelPanel.LoCrButton", MPA_SearchAndDelPanel,
    "DMPanelNoble:LoCrButtonTemplate");
...
MPA_SearchAndDelPanel.LoObjButton = CreateFrame("BUTTON", "MPA_SearchAndDelPanel.LoObjButton", MPA_SearchAndDelPanel,
    "DMPanelNoble:LoObjButtonTemplate");
...
MPA_SearchAndDelPanel.DeleteNPC.Icon = CreateFrame("BUTTON", nil, MPA_SearchAndDelPanel.DeleteNPC,
    "DMPanelNoble:MiniIconButtonTemplate");
...
MPA_SearchAndDelPanel.GobDelRadiusButton = CreateFrame("Button", "MPA_SearchAndDelPanel.GobDelRadiusButton",
    MPA_SearchAndDelPanel, "DMPanelNoble:MiniIconButtonBG")
...
MPA_SearchAndDelPanel.GobDelRadiusButton.Icon:SetScript("OnClick", function()
    DMPanelNoble:WarningFrameShow(Chosen_GobDelRadius)
end)
...
MPA_AurasAndStatesPanel.Morph = CreateFrame("BUTTON", "MPA_AurasAndStatesPanel.Morph", MPA_AurasAndStatesPanel,
    "DMPanelNoble:MorphButtonTemplate");
...
MPA_AurasAndStatesPanel.DeMorph = CreateFrame("BUTTON", "MPA_AurasAndStatesPanel.DeMorph", MPA_AurasAndStatesPanel,
    "DMPanelNoble:DeMorphButtonTemplate");
...
MPA_AurasAndStatesPanel.Phase.Icon = CreateFrame("BUTTON", nil, MPA_AurasAndStatesPanel.Phase,
    "DMPanelNoble:MiniIconButtonTemplate");
...
--- - - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -
--- - - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -
MPA_AurasAndStatesPanel.FlyOnOff = CreateFrame("Button", "MPA_AurasAndStatesPanel.FlyOnOff", MPA_AurasAndStatesPanel,
    "DMPanelNoble:MiniIconButtonBG")
MPA_AurasAndStatesPanel.FlyOnOff:SetPoint("CENTER", 80, 0)

MPA_AurasAndStatesPanel.FlyOnOff.Icon = CreateFrame("BUTTON", nil, MPA_AurasAndStatesPanel.FlyOnOff,
    "DMPanelNoble:MiniIconButtonTemplate");
...
MPA_AurasAndStatesPanel.FlyOnOff.Icon:SetScript("OnClick", function()
    if DMLevel < 3 or not UnitAura("player", "Творческая фаза") then
        print("|cffff9716[ДМ-аддон]: Вы должны находиться в творческой фазе.|r")
        return
    end
...
--- - - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -
--- - - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -
MPA_AurasAndStatesPanel.InvisOnoff = CreateFrame("Button", "MPA_AurasAndStatesPanel.InvisOnoff",
    MPA_AurasAndStatesPanel, "DMPanelNoble:MiniIconButtonBG")
MPA_AurasAndStatesPanel.InvisOnoff:SetPoint("CENTER", 80, -35)

local InvisModif = 0
MPA_AurasAndStatesPanel.InvisOnoff.Icon = CreateFrame("BUTTON", nil, MPA_AurasAndStatesPanel.InvisOnoff,
    "DMPanelNoble:MiniIconButtonTemplate");
...
--- - - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -
--- - - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -///- - - - - - - - - - - -
MPA_AurasAndStatesPanel.SpeedOnOff = CreateFrame("Button", "MPA_AurasAndStatesPanel.SpeedOnOff",
    MPA_AurasAndStatesPanel, "DMPanelNoble:MiniIconButtonBG")
MPA_AurasAndStatesPanel.SpeedOnOff:SetPoint("CENTER", 120, -35)

local SeedUpModif = 0
MPA_AurasAndStatesPanel.SpeedOnOff.Icon = CreateFrame("BUTTON", nil, MPA_AurasAndStatesPanel.SpeedOnOff,
    "DMPanelNoble:MiniIconButtonTemplate");
...
MPA_ControlPanel.RollButton = CreateFrame("BUTTON", "MPA_ControlPanel.RollButton", MPA_ControlPanel,
    "DMPanelNoble:BattleButtonTemplate");
...
MPA_ControlPanel.Summon.Icon =
    CreateFrame("BUTTON", nil, MPA_ControlPanel.Summon, "DMPanelNoble:MiniIconButtonTemplate");
...
MPA_ControlPanel.Waypoint = CreateFrame("BUTTON", "MPA_ControlPanel.Waypoint", MPA_ControlPanel,
    "DMPanelNoble:WaypointButtonTemplate");
...
SettingsFrame.NPCSay:SetScript("OnClick", function()
    ...
    if DMPanelNoble.db.profile.settings.NPCSayByEmote then
        SettingsFrame.NPCSay:SetChecked(true)
    else
        SettingsFrame.NPCSay:SetChecked(false)
    end
...
SettingsFrame.NPCTalkAnimation = CreateFrame("CheckButton", "SettingsFrame_NPCSay", SettingsFrame.AddName,
    "ChatConfigCheckButtonTemplate");
SettingsFrame.NPCTalkAnimation:SetChecked()
SettingsFrame.NPCTalkAnimation:SetPoint("BOTTOMLEFT", SettingsFrame.SaveFocusEditbox, "TOPLEFT", 0, 25);
...
SettingsFrame.NPCTalkAnimation:SetScript("OnClick", function()
    DMPanelNoble.db.profile.settings.NPCTalkAnimation = not DMPanelNoble.db.profile.settings.NPCTalkAnimation
    SettingsFrame.NPCTalkAnimation:SetChecked(DMPanelNoble.db.profile.settings.NPCTalkAnimation)
end);
